...
\begin{itemize}
 ...
 \item S Phi vee Osi in H, Phi in h ou Psi in H
 \item Si lnot Phi vee psi in H, Phi not in H et Psi not in H
...
M, Pi models Phi ssi M, Pi_0 models_exists Phi
		 ssi M, Pi_0 models_forall Phi
\end{rem}

\begin{thm}
 Le model checking de CTL sur M = W,R,V et Phi prend O(|Phi|  times ( | X| + |R|) opérations.
\end{thm}

\begin{ex}
 Phi_G = \bigvee_{sigma permutation de 1 n} Phi_sigma
où phi_sigma = 
\end{ex}
...
Phi_1 equivalente à Phi_2 ssi pour tout modèle M,s, on a 
	    M,s models_forall Phi_1 ssi M,s models_forall Phi_2
...
\begin{lem}
forall n, forall M,
 forall Phi LTL, si Phi leq n, exists x, y subseteq Pi tq
...
\begin{dem}[du théorème]
 ...
 M,s models_exists Phi ssi forall Pi qui part de s, on a M, Pi models chi
		       ...
		       où M^Pi' = (W', R', V') avec W' = {0, dots |xy|}
						    R' = {(n, n+1) tq n < |xy|} cup {(|xy|, |x|)}
						    V'(i) = V(Pi_i)
		       ssi forall Pi' dots on a M^Pi', 0 models_exists Phi
		       ssi forall Pi' dots on a M^Pi', 0 models_exists Psi (comme M^Pi déterministe, on peut supprimer les forall et exists de Phi)
		       ssi forall Pi' = x y ^omega, M, Pi' models Psi
		       ssi forall Pi chemin quelconque, M, Pi models_forall Psi (en utilisant le lemme)
\end{dem}
...
M, w not models_forall G F p
M, w not models_exists forall G exists F p
\end{dem}
...
Pour tout n in N,
M, n s_n not models forall FGp
M'n, s'n models_forall F G p

\begin{lem}
 forall n in N, pour toute formmule de CTl, Phi tq | Phi | leq n, Mn, sn models Phi ssi M'n s'n models Phi
\end{lem}
...
M,w models_K Phi ssi M,w models_forall,exists tr(Phi)
...
\begin{definition}
 On dit que M, Pi et M', Pi' sont bissimilaires  ssi forall n in N, 
M, Pi_n bissim M', Pi_n'
\end{definition}

\begin{prop}
 Soient M, w bisim M',w, soit Pi un chemin qui part de w dans M.
 Alors exists Pi' chemin partant de w' tq M, Pi bisim M', Pi'
\end{prop}
\begin{dem}
...
On a équivalence entre :
  \begin{enumerate}
   \item M, w bisim M', w'
   \item forall Phi de CTL*, M,w models_exists Phi ssi M', w' models_exists Phi
   \item forall Phi de CTL, M,w models_exists Phi ssi M', w' models_exists Phi
  \end{enumerate}
