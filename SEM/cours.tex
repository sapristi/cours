...
séquence : on suppose models \models \semh{A}{P}{C}^{(1)} et \models \semh{C}{Q}{B}^{(2)}; on vérifie \models \semh{A}{P;Q}{B}  
...
A partir d'un programme annoté P^*, on peut retrer toute assertion non étiqutée par inv
...
\begin{cor}
 si il existe A_0 tel que WP(P,B) = \semm{A_0}, alors models \semh{A}{P}{B} ssi models A Rightarrow A_0
\end{cor}

Question : un tel A_0 existe-t-il toujours ? → définissabilité

WP(skip,B) = \semm{B}
WP(x:=a,B) = \{ sigma in Etat_bot tq \semm{x := a} sigma models B\}
	   = \{ sigma in Etat tq \semm{x := a} sigma models B\} \cup \{bot\} = \semm{B[a/x]}
...
WP(while b do P; Q) = (WP(P, WP(while dots , B)) cap \{sigma in Etat tq semm{b} = tt \})
		      cup (semm{B} cap \{sigma in Etat tq semm b sigma ) ff \}
		      cup {bot}

Il faut exprimer les itérations : 
sigma in WP(while dot, B) ssi forall k in N , forall sigma_0, dots, sigma_k in Etat, 
...
\begin{thm}
 pour tout programme P et toute assertion B, il existe une assertion W(P,B) tq semm{W(P,B)} = WP(P,B)
\end{thm}

\section{Complétude}
On veut prouver models \semh{A}{P}{B} alors vdash \semh{A}{P}{B}
...
Par hypothèse d'induction, vdash \semh{W(P_1,B)}{P_2}{B}
	  ...
	  donc(affaibl) vdash \semh{W(P_1;P_2,B)}{P_1;P_2}{B}
