...
$c \sim^V_{res}$ ssi $\forall \sigma \quad \semm{c}_{res} \sigma \sim^V \semm{c}_res \sigma$
...
\begin{itemize}
 ...
 \item choix : $\mathcal{C}\semm{\text{if v then P else Q}} c = c'\text{ avec }c' \sigma =\text{ si }\semm{b} \sigma = tt \text{ alors } (\mathcal{C}\semm{P} c) \sigma \text{ sinon } (\mathcal{C}\semm{Q} c) \sigma$\\
